...
IMA Agent is the main application implementing connectivity and message transfer between `Main Net` and `S-Chain`. Agent also provides easy way to do ETH money, ERC20 and ERC721 transfers between `Main Net` and `S-Chain` nevertheless this can be done without it.
...
1) Prepare needed prerequisites
...
5) Truffle version **5.0.12** is recommended to install (notice, the *-g* option of *npm* may require *sudo*):
```bash
