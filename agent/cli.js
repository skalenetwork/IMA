...
function parse( joExternalHandlers ) {
    ...
    for ( idxArg = 2; idxArg < cntArgs; ++idxArg ) {
        ...
        if ( joArg.name == "help" ) {
            ...
            //
            console.log( soi + cc.debug( "--" ) + cc.bright( "key-main-net" ) + cc.sunny( "=" ) + cc.error( "value" ) + cc.debug( "............" ) + cc.notice( "Private key for " ) + cc.note( "main-net user" ) + cc.notice( " account address. Value is automatically loaded from the " ) + cc.warning ("PRIVATE_KEY_FOR_MAINNET" ) + cc.notice( " environment variable if not specified." ) );
