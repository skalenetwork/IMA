...
export function find_pow_number( strRequestPart, details, isVerbose ) {
    ...
    if( isVerbose ) {
        details.write(
            cc.debug( "source part of request to find " ) + cc.sunny( "PoW number" ) +
    ...
    if( isVerbose ) {
        details.write(
            cc.debug( "source " ) + cc.sunny( "t" ) + cc.debug( "=" ) +
        ...
        if( r.gt( g_bnMIN_POW_RESULT ) ) {
            if( isVerbose ) {
                details.write(
                    ...
                    cc.info( i ) + cc.debug( ", this is resulting " ) + cc.sunny( "PoW number" ) +
                    "\n" );
                details.write(
    ...
    const promise_complete = new Promise( ( resolve, reject ) => {
        try {
            ...
            rpcCall.create( url, callOpts || { }, async function( joCall, err ) {
                if( err ) {
                    details.write(
                        cc.fatal( "CRITICAL ORACLE CALL ERROR:" ) +
                ...
                try {
                    ...
                    if( isVerboseTraceDetails ) {
                        details.write(
                            cc.debug( "RPC call" ) + cc.normal( "(" ) +
                            cc.attention( "oracle_submitRequest" ) + cc.normal( ")" ) +
                            cc.debug( " is " ) + cc.j( joIn ) +
                            "\n" );
                    }
                    await joCall.call( joIn, async function( joIn, joOut, err ) {
                        ...
                        for( let idxAttempt = 0; idxAttempt < cntAttempts; ++idxAttempt ) {
                            ...
                            try {
                                ...
                                await joCall.call( joIn, async function( joIn, joOut, err ) {
                                    ...
                                    if( isVerboseTraceDetails ) {
                                        details.write(
                                            cc.debug( "RPC call" ) + cc.normal( "(" ) +
                                            cc.attention( "oracle_checkResult" ) +
                                            cc.normal( ")" ) + cc.debug( " parsed " ) +
                                            cc.sunny( "result" ) + cc.debug( " field is: " ) +
                                            cc.j( joResult ) +
                                            "\n" );
                                    }
                                ...
                                details.write(
                                    cc.fatal( "CRITICAL ORACLE CALL ERROR:" ) +
                                    cc.error( " RPC call" ) +
                                    cc.normal( "(" ) + cc.attention( "oracle_checkResult" ) +
                                    cc.normal( ")" ) +
                        ...
                        details.write(
                            ...
                            cc.normal( "(" ) + cc.attention( "oracle_checkResult" ) +
                            cc.normal( ")" ) + cc.error( " all attempts timed out" ) +
                            "\n" );
                        reject( new Error(
