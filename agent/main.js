...
if( tmp_address_MN_from_env && typeof tmp_address_MN_from_env == "string" && tmp_address_MN_from_env.length > 0 ) { imaState.joAccount_main_net.address_ = "" + tmp_address_MN_from_env; }
if( tmp_address_SC_from_env && typeof tmp_address_SC_from_env == "string" && tmp_address_SC_from_env.length > 0 ) { imaState.joAccount_s_chain.address_ = "" + tmp_address_SC_from_env; }
        ...
        imaState.arrActions.push( {
            ...
            fn: async function() {
                if( !await check_registration_step1() ) {
                    if( !await register_step1() ) { return false; }
                }
                if( !await check_registration_step2() ) {
                    if( !await register_step2() ) { return false; }
                }
                if( !await check_registration_step3() ) {
                    if( !await register_step3() ) { return false; }
                }
...
if( imaState.bIsNeededCommonInit ) { imaCLI.ima_common_init(); }
...
async function discover_s_chain_network( fnAfter ) {
    ...
    await rpcCall.create( imaState.strURL_s_chain, async function( joCall, err ) {
        ...
        await joCall.call( {
            ...
            if( IMA.verbose_get() >= IMA.RV_VERBOSE.trace ) { log.write( strLogPrefix + cc.normal( "OK, got S-Chain network information: " ) + cc.j( joOut.result ) + "\n" ); } else if( IMA.verbose_get() >= IMA.RV_VERBOSE.information ) { log.write( strLogPrefix + cc.success( "OK, got S-Chain network information." ) + "\n" ); }
            let nCountReceivedImaDescriptions = 0;
            ...
            for( let i = 0; i < jarrNodes.length; ++i ) {
                ...
                await rpcCall.create( strNodeURL, function( joCall, err ) {
                    ...
                    joCall.call( {
                        ...
                        if( IMA.verbose_get() >= IMA.RV_VERBOSE.information ) { log.write( strLogPrefix + cc.success( "OK, got node " ) + cc.info( joNode.nodeID ) + cc.success( " IMA information(" ) + cc.info( nCountReceivedImaDescriptions ) + cc.success( " of " ) + cc.info( jarrNodes.length ) + cc.success( ")." ) + "\n" ); }
                    } );
                } );
            }
            // process.exit( 0 );
...
async function do_the_job() {
    ...
    let idxAction; const cntActions = imaState.arrActions.length;
    let cntFalse = 0;
    let cntTrue = 0;
    for( idxAction = 0; idxAction < cntActions; ++idxAction ) {
        ...
        try {
            if( await joAction.fn() ) {
                ++cntTrue;
                if( IMA.verbose_get() >= IMA.RV_VERBOSE.information ) { log.write( strLogPrefix + cc.success( "Succeeded action:" ) + " " + cc.info( joAction.name ) + "\n" ); }
            } else {
                ++cntFalse;
                if( IMA.verbose_get() >= IMA.RV_VERBOSE.error ) { log.write( strLogPrefix + cc.warning( "Failed action:" ) + " " + cc.info( joAction.name ) + "\n" ); }
            }
        } catch ( e ) {
            ++cntFalse;
            if( IMA.verbose_get() >= IMA.RV_VERBOSE.fatal ) { log.write( strLogPrefix + cc.fatal( "CRITICAL ERROR: Exception occurred while executing action:" ) + " " + cc.info( joAction.name ) + cc.error( ", error description: " ) + cc.warning( e ) + "\n" ); }
        }
    } // for( idxAction = 0; idxAction < cntActions; ++ idxAction )
    if( IMA.verbose_get() >= IMA.RV_VERBOSE.information ) {
...
if( imaState.bSignMessages ) {
    ...
    discover_s_chain_network( function( err, joSChainNetworkInfo ) {
        if( err ) { process.exit( 1 ); } // error information is printed by discover_s_chain_network()
        if( IMA.verbose_get() >= IMA.RV_VERBOSE.information ) { log.write( cc.success( "S-Chain network was discovered: " ) + cc.j( joSChainNetworkInfo ) + "\n" ); }
...
async function register_step1() {
    ...
    const bRetVal = !!( ( bRetVal1A && bRetVal1B ) );
    if( !bRetVal ) {
        const nRetCode = 1501;
...
function check_time_framing( d ) {
    try {
        if( imaState.nTimeFrameSeconds <= 0 || imaState.nNodesCount <= 1 ) { return true; } // time framing is disabled
        if( d == null || d == undefined ) { d = new Date(); } // now
        ...
        if( IMA.verbose_get() >= IMA.RV_VERBOSE.fatal ) { log.write( cc.fatal( "Exception in check_time_framing():" ) + cc.error( e ) + "\n" ); }
    }
    return true;
}
...
async function single_transfer_loop() {
    ...
    if( IMA.verbose_get() >= IMA.RV_VERBOSE.debug ) { log.write( strLogPrefix + cc.debug( IMA.longSeparator ) + "\n" ); }
    if( !check_time_framing() ) {
        if( IMA.verbose_get() >= IMA.RV_VERBOSE.debug ) { log.write( strLogPrefix + cc.warning( "Skipped due to time framing" ) + "\n" ); }
        return true;
    }
    ...
    const b3 = b1 && b2;
    if( IMA.verbose_get() >= IMA.RV_VERBOSE.information ) { log.write( strLogPrefix + cc.debug( "Completed: " ) + cc.tf( b3 ) + "\n" ); }
    return b3;
}
...
async function run_transfer_loop( isDelayFirstRun ) {
    ...
    if( isDelayFirstRun ) { setTimeout( single_transfer_loop_with_repeat, imaState.nLoopPeriodSeconds * 1000 ); } else { await single_transfer_loop_with_repeat(); }
    return true;
}
