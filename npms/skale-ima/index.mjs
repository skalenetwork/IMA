...
export async function safe_getPastEventsProgressive( details, ethersProvider, attempts, joContract, strEventName, nBlockFrom, nBlockTo, joFilter ) {
    ...
    for( let idxPlan = 0; idxPlan < arr_progressive_events_scan_plan.length; ++idxPlan ) {
        ...
        details.write(
            ...
            cc.debug( "..." ) + "\n" );
        try {
            const joAllEventsInBlock = await safe_getPastEventsIterative( details, ethersProvider, attempts, joContract, strEventName, joPlan.nBlockFrom, joPlan.nBlockTo, joFilter );
...
export async function get_contract_call_events( details, ethersProvider, joContract, strEventName, nBlockNumber, strTxHash, joFilter ) {
    ...
    if( nBlockTo > nLatestBlockNumber )
        nBlockTo = nLatestBlockNumber;
    const joAllEventsInBlock = await safe_getPastEventsIterative( details, ethersProvider, 10, joContract, strEventName, nBlockFrom, nBlockTo, joFilter );
...
export async function checkTransactionToSchain(
    ...
    const requiredBalance = txx.gasPrice * txx.gasLimit;
    const balance = await ethersProvider.getBalance( sender );
    if( balance < requiredBalance ) {
        details.write(
            cc.warning( "Insufficient funds for " ) + cc.bright( sender ) +
            cc.warning( "; Will run " ) + cc.sunny( "PoW" ) + cc.warning( " for mining " ) +
            cc.bright( txx.gasLimit ) + cc.warning( " gas" ) +
            "\n" );
        const powNumber = await calculatePowNumber( sender, txx.nonce, txx.gasLimit, details );
        details.write(
            cc.warning( "Done, " ) + cc.sunny( "PoW" ) +
            cc.warning( " number is " ) + cc.bright( powNumber ) +
            "\n" );
        txx.gasPrice = owaspUtils.ensure_starts_with_0x( powNumber );
    }
    return txx;
}
...
async function find_out_reference_log_record( details, ethersProvider, jo_message_proxy, nBlockId, nMessageNumberToFind, isVerbose ) {
    ...
    const arrLogRecords = await safe_getPastEventsProgressive(
        details,
        ethersProvider,
        10,
        jo_message_proxy,
...
async function find_out_all_reference_log_records( details, ethersProvider, jo_message_proxy, nBlockId, nIncMsgCnt, nOutMsgCnt, isVerbose ) {
    ...
    for( ; nWalkMsgNumber >= nIncMsgCnt; -- nWalkMsgNumber ) {
        const joReferenceLogRecord = await find_out_reference_log_record( details, ethersProvider, jo_message_proxy, nWalkBlockId, nWalkMsgNumber, isVerbose );
