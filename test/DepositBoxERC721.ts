...
describe("DepositBoxERC721", () => {
    ...
    beforeEach(async () => {
        ...
        await messageProxy.registerExtraContractForAll(await depositBoxERC721.getAddress());
    ...
    describe("tests with `ERC721`", async () => {
        ...
        describe("tests for `depositERC721` function", async () => {
            ...
            it("should rejected with `Unconnected chain`", async () => {
                ...
                await depositBoxERC721.connect(user).depositERC721(schainName, await erc721.getAddress(), amount)
            ...
            it("should invoke `depositERC721` without mistakes", async () => {
                ...
                await erc721OnChain.connect(deployer).approve(await depositBoxERC721.getAddress(), tokenId);
                await erc721OnChain.connect(deployer).approve(await depositBoxERC721.getAddress(), tokenId2);
                // execution
                await depositBoxERC721
                    .connect(deployer)
                ...
                // expectation
                expect(await erc721OnChain.ownerOf(tokenId)).to.equal(await depositBoxERC721.getAddress());
                expect(await erc721OnChain.ownerOf(tokenId2)).to.equal(await depositBoxERC721.getAddress());
            });
            ...
            it("should invoke `depositERC721Direct` without mistakes", async () => {
                ...
                await depositBoxERC721.connect(user).disableWhitelist(schainName);
                const data1 = await messages.encodeTransferErc721AndTokenInfoMessage(contractHere, to, tokenId, { name: "ERC721OnChain", symbol: "ERC721" });
                const data2 = await messages.encodeTransferErc721Message(contractHere, to, tokenId2);
                await depositBoxERC721
                    .connect(deployer)
                ...
                await depositBoxERC721
                    ...
                    .should.emit(messageProxy, "OutgoingMessage")
                    .withArgs(schainHash, 1, await depositBoxERC721.getAddress(), deployer.address, data2);
                // console.log("Gas for depositERC721:", res.receipt.gasUsed);
                // expectation
                expect(await erc721OnChain.ownerOf(tokenId)).to.equal(await depositBoxERC721.getAddress());
                expect(await erc721OnChain.ownerOf(tokenId2)).to.equal(await depositBoxERC721.getAddress());
        ...
        it("should get funds after kill", async () => {
            ...
            await depositBoxERC721.connect(user).getFunds(schainName, await erc721OnChain.getAddress(), user.address, tokenId).should.be.eventually.rejectedWith("Schain is not killed");
            await linker.connect(deployer).kill(schainName);
            await linker.connect(user).kill(schainName);
            await depositBoxERC721.connect(user).getFunds(schainName, await erc721OnChain.getAddress(), user.address, tokenId2).should.be.eventually.rejectedWith("Incorrect tokenId");
            await depositBoxERC721.connect(user).getFunds(schainName, await erc721OnChain.getAddress(), user.address, tokenId);
        ...
        it("should add erc721 token by schain owner", async () => {
            ...
            await depositBoxERC721.connect(deployer).addERC721TokenByOwner(schainName, await erc721.getAddress())
                .should.be.eventually.rejectedWith("Sender is not an Schain owner");

            await depositBoxERC721.connect(user).addERC721TokenByOwner(schainName, await erc721.getAddress());
            await depositBoxERC721.connect(user).addERC721TokenByOwner(schainName, await erc721.getAddress()).should.be.eventually.rejectedWith("ERC721 Token was already added");
            expect(await depositBoxERC721.getSchainToERC721(schainName, await erc721.getAddress())).to.be.equal(true);
            expect((await depositBoxERC721.getSchainToAllERC721(schainName, 0, 1))[0]).to.be.equal(await erc721.getAddress());
    ...
    describe("tests for `postMessage` function", async () => {
        ...
        it("should transfer ERC721 token", async () => {
            ...
            const message = {
                data: await messages.encodeTransferErc721Message(await erc721.getAddress(), to, tokenId),
            ...
            await erc721.connect(deployer).transferFrom(deployer.address, await depositBoxERC721.getAddress(), tokenId);
        ...
        it("should transfer ERC721 token", async () => {
            ...
            await erc721.approve(await depositBoxERC721.getAddress(), tokenId);
            ...
            balance.should.not.be.lessThan(balanceBefore);
            balance.should.be.almost(balanceBefore);
            expect(await depositBoxERC721.transferredAmount(await erc721.getAddress(), tokenId)).to.be.equal(zeroHash);
