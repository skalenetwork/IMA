...
npx ganache-cli --gasLimit 9000000 --quiet --allowUnlimitedContractSize --account_keys_path "$DEPLOYED_DIR/proxy/accounts.json" &

cd "$DEPLOYED_DIR"
yarn install
cd proxy
PRIVATE_KEY_FOR_ETHEREUM=$(cat accounts.json | jq  '.private_keys | to_entries | .[8].value')
PRIVATE_KEY_FOR_SCHAIN=$(cat accounts.json | jq  '.private_keys | to_entries | .[9].value')
