...
if [ "${DIRECTION}" = main ]; then
    ...
    npx hardhat run migrations/deployMainnet.ts --network "${NETWORK_FOR_ETHEREUM}" || exit $?
    ...
    npx hardhat run migrations/deploySchain.ts --network "${NETWORK_FOR_SCHAIN}" || exit $?
