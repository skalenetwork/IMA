...
contract TokenManagerLinker is ITokenManagerLinker, AccessControlEnumerableUpgradeable {
    ...
    function setLinkerAddress(address newLinker) external {
        linkerAddress = newLinker;
    }
     ...
     */
    function hasTokenManager(address tokenManager) external view override returns (bool) {
        // TODO: optimize algorithm to save gas
        uint index;
        uint length = tokenManagers.length;
        for (index = 0; index < length; index++) {
            if (address(tokenManagers[index]) == tokenManager) {
                return true;
            }
        }
        return false;
