...
def calculate_mapping_value_slot(slot: int, key: any, key_type: str) -> int:
    if key_type == "address":
        return calculate_mapping_value_slot(
            slot, int(key, 16).to_bytes(32, "big"), "bytes32"
        )
    else:
        return int.from_bytes(
            Web3.solidity_keccak([key_type, "uint256"], [key, slot]), "big"
        )
...
class ContractGenerator:
    ...
    def _setup_role(
        self, roles_slot: int, role_members_slot: int, role: bytes, accounts: [str]
    ):
        role_data_slot = calculate_mapping_value_slot(roles_slot, role, "bytes32")
        members_slot = role_data_slot
        role_members_value_slot = calculate_mapping_value_slot(
            role_members_slot, role, "bytes32"
        )
        ...
        for i, account in enumerate(accounts):
            members_value_slot = calculate_mapping_value_slot(
                members_slot, account, "address"
            )
