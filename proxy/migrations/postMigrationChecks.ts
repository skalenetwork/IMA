...
async function checkMainnetIMA(
    ...
    for (const contract of contracts){
        ...
        if (await oldProvider.getBalance(oldAddress) !== await newProvider.getBalance(newAddress)) {
            console.log(`${contract} does not have the same balance on both Chains.`);
            if (await oldProvider.getBalance(oldAddress) >= ethers.parseEther("5") && await newProvider.getBalance(newAddress) == ethers.parseEther("5")) {
                const amount = await oldProvider.getBalance(oldAddress) - ethers.parseEther("5");
                console.log(`But it's expected as balance was > 5ETH. Top up ${contract} at ${newAddress} with missing ${ethers.formatEther(amount)} ETH`);
            }
            else{
                ok = false;
            }
