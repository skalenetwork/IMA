...
| If kill operation is executed, allows owner to return tokens to end-users.

| MessageProxyForMainnet
| registerExtraContract removeExtraContract
| Allows owner to add/remove extra contracts to the IMA Bridge.

.2+| Linker
| allowInterchainConnections

| Allows owner to connect other chains that they own to the bridge.

| kill
| Allows owner and contract deployer to kill the SKALE Chain and allow owner to execute getFunds(). Both roles must call this function to execute.
...
| registerExtraContract removeExtraContract
| Allows owner or `EXTRA_CONTRACT_REGISTRAR_ROLE` to add/remove an extra SKALE Chain contract to the IMA Bridge.
...
| Allows owner or `REGISTRAR_ROLE` to add/remove
...
| Allows owner or `CONSTANT_SETTER_ROLE` 
