...
export async function makeDeposit() {
  ...
  const rinkebyContract = new web3Rinkeby.eth.Contract(erc20ABI, erc20Address);
  await mainnet.addERC20Token(ERC20_NAME, rinkebyContract);

  // setup options and parameters
  let txOpts = {
    address: process.env.REACT_APP_INSECURE_ACCOUNT,
    privateKey: process.env.REACT_APP_INSECURE_PRIVATE_KEY
  };
  let address = process.env.REACT_APP_INSECURE_ACCOUNT;
...
export async function exit() {
  ...
  let sChainerc20ABI = schainERC20ABI.erc20_abi;
  let schainerc20Address = schainERC20ABI.erc20_address;
