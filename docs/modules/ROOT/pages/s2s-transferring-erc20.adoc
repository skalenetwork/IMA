...
⇢ Add the `target` chain using `connectSchain` within the **TokenManagerLinker** contract on the `origin` chain. 

⇠ To exit from the target, you must also reciprocate the connection by adding the `origin` chain using `connectSchain` on the `target` chain.
...
Now you need to assign the **TokenManagerERC20** contact on your SKALE Chain as the MINTER_ROLE and BURNER_ROLE for the ERC20 clone on the target. With OpenZeppelin's framework, you simply need to execute an AddMinter and AddBurner transaction on the SKALE chain token contract.
