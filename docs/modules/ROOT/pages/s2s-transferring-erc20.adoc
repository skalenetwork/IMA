...
=== 1. Connect the target to origin
...
=== 2. Connect the origin to target

Repeating step 1 but on the `target` chain, add the `origin` chain by using `connectSchain` within the **TokenManagerLinker** contract.
