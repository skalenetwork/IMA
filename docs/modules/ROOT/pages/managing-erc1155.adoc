...
[WARNING]
All IMA messages (ETH, ERC20, ERC721, ERC1155, and customMessages) have a set gas limit of 1000000. This limit may be breached when using `exitToMainERC1155Batch` or `transferToSchainERC1155Batch` when sending a large batch of transfers from SKALE Chain to Mainnet or SKALE Chain to SKALE Chain. To avoid loosing tokens, only use these functions for batches that fall under this gas limit.
