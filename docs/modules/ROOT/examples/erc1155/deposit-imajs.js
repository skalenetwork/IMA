...
export async function depositERC1155Single(ima) {
  ...
  let balanceSchain = await ima.schain.getERC1155Balance(schainERC1155, address, erc1155TokenId); // get balance on the sChain before the transfer

  await ima.mainnet.erc1155.approveAll(tokenName, opts);
  await ima.depositERC1155(schainName, tokenName, erc1155TokenId, amount, opts);
  await ima.schain.waitERC1155BalanceChange(schainERC1155, address, erc1155TokenId, balanceSchain);
}